...
my @felt_prefiks;
my @felt_prefiks_alle =("nettadr","maske","vlan","nettype","org","anv","samband","komm");
...
#	print "Start sammenlign $f\n";
	  ...
	  {
	      $_ = $prefiks{$f}[3]; #nettype
	      if(/lan/i) {
		  @felt_prefiks = ("nettadr","maske","vlan","nettype","org","anv","komm");
	      } elsif (/stam/i) {
		  @felt_prefiks = ("nettadr","maske","vlan","nettype","komm");
	      } elsif (/link/i) {
		  @felt_prefiks = ("nettadr","maske","vlan","nettype","samband","komm");
	      } elsif (/elink/i) {
		  @felt_prefiks = ("nettadr","maske","vlan","nettype","org","samband","komm");
	      } elsif (/loopback/i) {
		  @felt_prefiks = ("nettadr","maske","vlan","nettype");
	      }
	      
	      &sammenlikn_2(\%prefiks, \%db_prefiks, \@felt_prefiks, "prefiks", $f);
...
#oppdatereringer til null må ha egen spørring
		if ($ny{$f}[$i] eq "" && $gammel{$f}[$i] ne ""){
		    ...
		    $sql = "UPDATE $tabell SET $felt[$i]=null WHERE id=\'$id\'";
		    db_execute($sql,$conn);
		    print $sql;
		} else {
#normal oppdatering
		    print "\nOppdaterer $f felt $felt[$i] fra \"$gammel{$f}[$i]\" til \"$ny{$f}[$i]\"";
		    $sql = "UPDATE $tabell SET $felt[$i]=\'$ny{$f}[$i]\' WHERE id=\'$id\'";
		    print $sql;
		    db_execute($sql,$conn);
		}
	    }
...
#oppdatereringer til null må ha egen spørring
		if ($ny{$f}[$i] eq "" && $gammel{$f}[$i] ne ""){
		    print "\nOppdaterer $f felt $felt[$i] fra \"$gammel{$f}[$i]\" til \"NULL\"";
		    $sql = "UPDATE $tabell SET $felt[$i]=null WHERE $nokkel=\'$f\'";
		    db_execute($sql,$conn);
		    print $sql;
		} else {
#normal oppdatering
		    print "\nOppdaterer $f felt $felt[$i] fra \"$gammel{$f}[$i]\" til \"$ny{$f}[$i]\"";
		    $sql = "UPDATE $tabell SET $felt[$i]=\'$ny{$f}[$i]\' WHERE $nokkel=\'$f\'";
		    print $sql;
		    db_execute($sql,$conn);
		}
	    }
...
print	$tnett{$gwip}{nettadr} = &and_ip($gwip,$netmask);
print	$tnett{$gwip}{maske} = &mask_bits($netmask);
print	$tnett{$gwip}{prefiksid} = &hent_prefiksid($tnett{$gwip}{nettadr},
						   $tnett{$gwip}{maske});
	print "\n";
    }
...
#		    print "Sletter $gwip\n";
    ...
    foreach $gwip (sort by_ip keys %tnett)
    {
	my $id = $tnett{$gwip}{nettadr};
#	print $tnett{$gwip}{vlan} = &finn_vlan($tnett{$gwip}{interf});
#	print "\n";
...
#	if ($tnett{$gwip})  # lar være å legge inn de jeg akkurat har slettet:)
#	{
	$tnett{$gwip}{interf}   = $if{$tnett{$gwip}{indeks}}{interf};
	
#	print $tnett{$gwip}{prefiksid};
# = $if{$tnett{$gwip}{indeks}}{prefiksid};
	...
	$gwport{$gwip} = [$gwip,$boksid,$tnett{$gwip}{prefiksid},$tnett{$gwip}{indeks},$tnett{$gwip}{interf},$tnett{$gwip}{speed},$tnett{$gwip}{maxhosts},$tnett{$gwip}{antmask},$tnett{$gwip}{ospf},$tnett{$gwip}{hsrppri},"f"];
#	print $gwip."\t".$tnett{$gwip}{indeks}."\t".$tnett{$gwip}{interf}."\t".$tnett{$gwip}{ospf}."\t".$tnett{$gwip}{speed}."\t".$tnett{$gwip}{maxhosts}."\t".$tnett{$gwip}{antmask}."\n";
    }
