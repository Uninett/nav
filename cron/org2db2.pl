...
#select 
$sql = "SELECT ".join(",", @felt )." FROM org ORDER BY $felt[0]";
...
#oppdatereringer til null må ha egen spørring
		if ($fil{$f}[$i] eq "" && $db{$f}[$i] ne ""){
		    ...
		    $sql = "UPDATE org SET $felt[$i]=null WHERE $felt[0]=\'$f\'";
		    db_execute($sql,$conn);
		    print $sql;
		} else {
#normal oppdatering
		    print "\nOppdaterer $f felt $felt[$i] fra \"$db{$f}[$i]\" til \"$fil{$f}[$i]\"";
		    $sql = "UPDATE org SET $felt[$i]=\'$fil{$f}[$i]\' WHERE $felt[0]=\'$f\'";
		    print $sql;
		    db_execute($sql,$conn);
		}
	    }
...
#INSERT
	...
	$sql = "INSERT INTO org (".join(",",@kii).") VALUES (".join(",",@val).")";
	print $sql;
	db_execute($sql,$conn);
    }
...
for my $f (keys %db) {
    unless(exists($fil{$f})) {
	print "sletter ".$f;
	$sql = "DELETE FROM org WHERE $felt[0]=\'$f\'";
	db_execute($sql,$conn);
    }
}
sub db_connect {
