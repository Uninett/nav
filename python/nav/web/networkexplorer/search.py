...
def search_expand_swport(swportid=None, swport=None, scanned=[]):
    ...
    for port in Interface.objects.filter(
        to_interface__in=found_swports, gw_port_prefixes__isnull=False
    ):
        found_gwports.append(port)

    for port in Interface.objects.filter(
        to_netbox=swport.netbox, gw_port_prefixes__isnull=False
    ):
        found_gwports.append(port)

    for port in Interface.objects.filter(
        to_interface=swport, gw_port_prefixes__isnull=False
    ):
        found_gwports.append(port)
...
def search_expand_netbox(netboxid=None, netbox=None):
    ...
    for result in netbox.get_uplinks():
        if (
            ...
            and result['other'].gw_port_prefixes.count()
        ):
    ...
    gwports = ports.filter(gw_port_prefixes__isnull=False)
...
def sysname_search(sysname, exact=False):
    ...
    for gwport in interfaces.filter(gw_port_prefixes__isnull=False):
