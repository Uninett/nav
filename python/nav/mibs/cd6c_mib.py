...
class CD6CMib(MibRetriever):
    ...
    sensors = {
        ...
        'temp': [
            ('cduStatusPrimaryTempT1',
             'Primary temperature (T1) sensor value.'),
            ('cduStatusSecondaryTempT2a',
             'Secondary temperature (T2a) sensor value.'),
            ('cduStatusSecondaryTempT2b',
             'Secondary temperature (T2b) sensor value.'),
    ...
    def get_all_sensors(self):
        ...
        if not supported:
            self._logger.debug('CD6C mib not supported - returned %s',
                               supported)
            defer.returnValue([])
