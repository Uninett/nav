...
def _add_import_parser(subparsers):
    ...
    import_parser.add_argument(
        "--on-conflict",
        choices=[m.value for m in ConflictMode],
    ...
    import_parser.add_argument(
        "--name",
        "-n",
        help="Override dashboard name from the name defined in file",
    )
    import_parser.add_argument(
        "--missing-user-ok",
        action="store_true",
    ...
    if args.format == "json":
        ...
        rows = [
            (
                str(d.id),
                d.account.login,
                f'"{d.name}"' + (" (shared)" if d.is_shared else ""),
            )
            for d in dashboards
        ]
        if rows:
...
def cmd_import(args):
    ...
    if args.shared:
        result.dashboard.is_shared = True
