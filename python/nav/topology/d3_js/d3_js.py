...
def export_d3_js(G, files_dir='nx_js_d3_graph', graphname="nx_js_d3_graph",
                 group='group',
    ...
    """
     ...
     The function builds a folder, containing the graph's formatted JSON,
     the D3.js
     ...
     graphname : string, optional
         the name of the graph being save as JSON, will appears in directory
         as 'graphname.json'
     group : string, optional
         The name of the 'group' key for each node in the graph. This is used
          to
         assign nodes to exclusive partitions, and for node coloring if
         visualizing.
     width : int, optional
     ...
     >>> low = 0
     >>> high = 5
     >>> G.add_nodes_from(map(lambda i: (i, {'group': random.random_integers
     (low, high, 1)[0]}), G.nodes()))
     >>> G.add_edges_from(map(lambda e: (e[0], e[1],
     {'weight': random.random_integers(low+1, high, 1)[0]}), G.edges()))
     >>> d3_js.export_d3_js(G, files_dir="random_lobster",
     graphname="random_lobster_graph", node_labels=False)
     """
    ...
    d3_files = {'d3.js': d3_js, 'd3.geom.js': d3_geom, 'd3.layout.js': d3_layout
        , 'force.css': d3_css, 'LICENSE': d3_license}
