...
When you have multiple forms that perform different actions, each form needs its own CSRF token. This example shows two forms for dashboard operations - one for renaming and one for deleting.

.. code-block:: html

   <form id="form-rename-dashboard" method="post" action="{% url 'rename-dashboard' dashboard.pk %}">
       {% csrf_token %}
       <input type="text" name="dashboard-name" value="{{ dashboard.name }}">
       <input type="submit" value="Rename dashboard">
   </form>
