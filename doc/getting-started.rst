...
We recommend you at least change these two options in `nav.conf`
before running NAV:
...
NAV will *not* autodiscover your network.  You need to use the
SeedDB tool (found in the Toolbox) to seed the database with IP
addresses and SNMP communities to monitor.
