package no.ntnu.nav.getDeviceData.deviceplugins.CiscoSwCL3;
...
/**
 * DeviceHandler for collecting switch port data from CL3 switches.
 */

public class CiscoSwCL3 implements DeviceHandler
{
	...
	public int canHandleDevice(Netbox nb) {
		int v = nb.getTypegroup() != null && nb.getTypegroup().equals("cL3-sw") ? ALWAYS_HANDLE : NEVER_HANDLE;
		Log.d("CL3_CANHANDLE", "CHECK_CAN_HANDLE", "Can handle device: " + v);
		return v;
	}

	public void handleDevice(Netbox nb, SimpleSnmp sSnmp, ConfigParser cp, DataContainers containers) throws TimeoutException
	{
		Log.setDefaultSubsystem("CL3_DEVHANDLER");
		...
		processCL3(netboxid, ip, cs_ro, typegroup, type, sc);
	 ...
	 *
	 */
	private void processCL3(String netboxid, String ip, String cs_ro, String typegroup, String typeid, SwportContainer sc) throws TimeoutException
	{
		...
		String ifTrunkOid = ".1.3.6.1.4.1.9.9.46.1.6.1.1.13";
		...
		/*
			...
			try {
				...
				Log.w("PROCESS_CL3", "IndexOutOfBoundsException while fetching (serial|hw|sw): " + e.getMessage() );
				e.printStackTrace(System.err);
				return;
			}
			Log.d("PROCESS_CL3", "Module: " + modul + " Serial: " + serial + " Hw_ver: " + hw_ver +  " Sw_ver: " + sw_ver);
		*/
			// Get data
		/* Ikke miber enda
			sSnmp.setBaseOid(serialOid);
			...
			*/
			...
			for (int j=0; j < portIfList.size(); j++) {
				...
				*/
				...
				String trunk = (String)trunkMap.get(ifindex);
				...
				try {
					...
					Log.w("PROCESS_CL3", "netboxid: " + netboxid + " ifindex: " + ifindex + " NumberFormatException on port: " + modulport[1]);
					continue;
				}
				...
				try {
					...
					Log.w("PROCESS_CL3", "netboxid: " + netboxid + " ifindex: " + ifindex + " NumberFormatException on speed: " + speed);
					continue;
				}
				...
				try {
					...
					if (n == 1) {
						...
						errl("  processCL3: netboxid: " + netboxid + " ifindex: " + ifindex + " Unknown status code: " + n);
						continue;
					}
				} catch (NumberFormatException e) {
					errl("  processCL3: netboxid: " + netboxid + " ifindex: " + ifindex + " NumberFormatException for status code: " + operStatus);
					continue;
				}
				...
				try {
					...
					errl("  processCL3: netboxid: " + netboxid + " ifindex: " + special + " NumberFormatException for status code: " + duplex);
					continue;
				}
				*/
				...
				try {
					...
					errl("  processCL3: netboxid: " + netboxid + " ifindex: " + ifindex + " NumberFormatException for status code: " + trunk);
					continue;
				}

				String media = "";

				Log.d("PROCESS_CL3", "Added portData("+netboxid+"): ifindex: " + ifindex + " Modul: " + modul + " Port: " + port + " Link: " + link + " Speed: " + speed + " Duplex: " + duplexS + " Media: " + media);
				...
				if (trunkB == true){
				    if(vlanHex == null){
					Log.w("PROCESS_CL3", "Error, vlanHex not found for ifindex: " + ifindex);
					continue;
				    } else {
				    ...
				    if(vlan == null){
					Log.w("PROCESS_CL3", "Error, vlan not found for ifindex: " + ifindex);
					continue;
				    } else {
					Integer vlanNum;
					try {
					    ...
					    Log.w("PROCESS_CL3", "netboxid: " + netboxid + " ifindex: " + ifindex + " NumberFormatException on vlan: " + vlan);
					    continue;
					}
