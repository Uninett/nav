package no.ntnu.nav.getDeviceData.deviceplugins.CiscoSwCGSW;
...
/**
 * DeviceHandler for collecting switch port data from CGSW switches.
 */

public class CiscoSwCGSW implements DeviceHandler
{
	...
	public int canHandleDevice(Netbox nb) {
		int v = nb.getTypegroup() != null && nb.getTypegroup().equals("cgsw-sw") ? ALWAYS_HANDLE : NEVER_HANDLE;
		Log.d("CGSW_CANHANDLE", "CHECK_CAN_HANDLE", "Can handle device: " + v);
		return v;
	}

	public void handleDevice(Netbox nb, SimpleSnmp sSnmp, ConfigParser cp, DataContainers containers) throws TimeoutException
	{
		Log.setDefaultSubsystem("CGSW_DEVHANDLER");
		...
		processCGSW(netboxid, ip, cs_ro, typegroup, type, sc);
	 ...
	 *
	 */
	private void processCGSW(String netboxid, String ip, String cs_ro, String typegroup, String typeid, SwportContainer sc) throws TimeoutException
	{
		...
		// Bruker egentlig denne status: .1.3.6.1.4.1.9.5.1.4.1.1.6, men forskjellen er forsvinnende liten
		...
		/*
			...
			try {
				...
				Log.w("PROCESS_CGSW", "IndexOutOfBoundsException while fetching (serial|hw|sw): " + e.getMessage() );
				e.printStackTrace(System.err);
				return;
			}
			Log.d("PROCESS_CGSW", "Module: " + modul + " Serial: " + serial + " Hw_ver: " + hw_ver +  " Sw_ver: " + sw_ver);
		*/
			// Get data
			sSnmp.setBaseOid(serialOid);
			...
			ArrayList ifPortList = sSnmp.getAll();
			...
			*/
			...
			if (ifSpeedList.size() != ifOperStatusList.size() || ifSpeedList.size() != portTypeList.size()) {
				errl("processCGSW: Size mismatch! ifSpeed: " + ifSpeedList.size() + ", ifOperStatus: " + ifOperStatusList.size() + ", portType: " + portTypeList.size());
				break;
			}
			*/
			...
			HashMap ifPortMap = new HashMap();
			for (int j=0; j < ifPortList.size(); j++) {
			    String[] portif = (String[])ifPortList.get(j);
			    ifPortMap.put(portif[0],portif[1]);
			}
			...
			for (int j = 0; j < moduleSerialList.size(); j++) {
			    ...
			    Log.d("PROCESS_CGSW", "Module: " + modul + " Serial: " + serial + " Hw_ver: " + hw +  " Sw_ver: " + sw);
			...
			for (int j=0; j < portIfList.size(); j++) {
				...
				*/
				...
				String modport = (String)ifPortMap.get(ifindex);
				String[] modulport = modport.split("/");
				...
				if (m == null) {
				    errl("  processCGSW: netboxid: " + netboxid + " ifindex: " + ifindex + " Could not find module: " + modul);
				    continue;
				}
				Integer vlanNum;
				try {
				...
				try {
					...
					Log.w("PROCESS_CGSW", "netboxid: " + netboxid + " portif: " + portif + " NumberFormatException on portif: " + portif);
					continue;
				}
				...
				try {
					...
					Log.w("PROCESS_CGSW", "netboxid: " + netboxid + " ifindex: " + ifindex + " NumberFormatException on speed: " + speed);
					continue;
				}
				...
				try {
					...
					if (n == 1) {
						...
						errl("  processCGSW: netboxid: " + netboxid + " ifindex: " + ifindex + " Unknown status code: " + n);
						continue;
					}
				} catch (NumberFormatException e) {
					errl("  processCGSW: netboxid: " + netboxid + " ifindex: " + ifindex + " NumberFormatException for status code: " + operStatus);
					continue;
				}
				...
				try {
					...
					errl("  processCGSW: netboxid: " + netboxid + " portif: " + portif + " NumberFormatException for status code: " + duplex);
					continue;
				}
				...
				try {
					...
					errl("  processCGSW: netboxid: " + netboxid + " portif: " + portif + " NumberFormatException for status code: " + trunk);
					continue;
				}
				String media = "";

				Log.d("PROCESS_CGSW", "Added portData("+netboxid+"): ifindex: " + ifindex + " Modul: " + modul + " Port: " + port + " Link: " + link + " Speed: " + speed + " Duplex: " + duplexS + " Media: " + media);
