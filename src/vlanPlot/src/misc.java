...
*/
	...
	************************************************************/
/*
	public static int[] findInSameSubnet(String InIp, String bits, String[] otherIps)
	{
		IP ip = new IP(InIp);
		String mask = IP.getMaskFromBits(Integer.parseInt(bits) );

		return ip.inSameSubnet(otherIps, mask);
	}
	...
	public static void quickSort(Vector v) {
		...
		while (incr >= 1) {
			for (int i = incr; i < v.size(); i++) {
				...
				//while (j >= incr && tmp.compareTo( v.elementAt(j - incr) ) < 0 )
	...
	public static void quickSort(String[] s)
	{
		...
		while (incr >= 1)
		{
			for (int i = incr; i < s.length; i++)
			{
				...
				while (j >= incr && compare(tmp, s[j - incr]) < 0 )
				{
					//v.setElementAt(v.elementAt(j - incr), j);
					s[j] = s[j - incr];
					j -= incr;
