...
def main():
    ...
    # `gh` does not appear to return null `baseRefName` in practice, but the
    # `.get(..., "")` default keeps this working if that ever changes.
    stable_prs = [pr for pr in prs if branch_re.match(pr.get("baseRefName", ""))]
...
def collect_issue_links(prs):
    ...
    for pr in prs:
        issue_numbers = set()
        # `gh` returns `[]` rather than null when there are no references,
        # but the `.get(..., [])` default keeps this working if that changes.
...
if __name__ == "__main__":
    try:
        ...
        if err.filename == "gh":
            sys.stderr.write(
                "The `gh` CLI is required but was not found in PATH.\n"
