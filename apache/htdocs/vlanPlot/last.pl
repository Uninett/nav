...
#	my $type = $FORM{'type'};
	...
	print "listSwitchlast\n";
	print "listSwitchPortlast\n";
    }
...
sub finnRuterLast {
	...
	if ($getInfo->ntuples == 0) {
		...
		$linkteller++;
	    } elsif ($linktype eq "stam") {
		push @{$stamliste[$stamteller]}, $_, $ruterid, $link, $linktype,$speed;
		$stamteller++;
	    } elsif ($linktype eq "lan") {
		push @{$lanliste[$lanteller]}, $_, $ruterid, $link, $linktype,$speed;
		$lanteller++;
	    } elsif ($linktype eq "elink") {
		push @{$elinkliste[$elinkteller]}, $_, $ruterid, $link, $linktype,$speed;
		$elinkteller++;
	    }
...
#		print "Finner ikke ruter med id $ruterid\n";
    ...
    my $stamliste = @stamliste;
    my $linkliste = @linkliste;
    my $lanliste = @lanliste;
    my $elinkliste = @elinkliste;
