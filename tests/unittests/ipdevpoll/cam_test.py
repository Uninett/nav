...
class TestPromoteHybridPorts:
    ...
    def test_when_flag_is_off_then_it_should_not_call_promote(self, cam_plugin):
        cam_plugin.linkports = {1: {'aa:bb:cc:dd:ee:01'}}
        cam_plugin.accessports = {}

        with patch.object(Cam, '_log_unmanaged_neighbor_macs', False):
            with patch.object(cam_plugin, '_promote_hybrid_ports') as mock_promote:
                # Directly test the conditional from handle()
                if cam_plugin._log_unmanaged_neighbor_macs:
                    cam_plugin._promote_hybrid_ports()
