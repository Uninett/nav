...
from nav.models.manage import Netbox, Room, NetboxInfo, Interface
...
class TestPatchModalViews:
    ...
    def test_should_render_add_patch_modal(self, client):
        url = reverse('seeddb-show-patch-modal') + '?interfaceid=123&modal=add'
        response = client.get(url)
        assert response.status_code == 200
        assert 'Add patch' in smart_str(response.content)

    def test_add_patch_modal_should_include_interface_id(self, client):
        url = reverse('seeddb-show-patch-modal') + '?interfaceid=123&modal=add'
        response = client.get(url)
        assert 'value="123"' in smart_str(response.content)

    def test_add_patch_modal_should_include_cable_search(self, client):
        url = reverse('seeddb-show-patch-modal') + '?interfaceid=123&modal=add'
        response = client.get(url)
        assert 'id="cable-search"' in smart_str(response.content)

    def test_add_patch_modal_should_include_cableid(self, client):
        url = reverse('seeddb-show-patch-modal') + '?interfaceid=123&modal=add'
        response = client.get(url)
        assert 'name="cableid"' in smart_str(response.content)

    def test_should_render_remove_patch_modal(self, client):
        url = reverse('seeddb-show-patch-modal') + '?interfaceid=123&modal=remove'
        response = client.get(url)
        assert response.status_code == 200
        assert 'Remove patch' in smart_str(response.content)

    def test_remove_patch_modal_should_include_interface_id(self, client):
        url = reverse('seeddb-show-patch-modal') + '?interfaceid=123&modal=remove'
        response = client.get(url)
        assert 'value="123"' in smart_str(response.content)
