...
class TestImportFromDict:
    ...
    def test_when_name_override_is_empty_then_it_should_raise(self, account):
...
class TestListDashboards:
    ...
    def test_when_no_filter_then_it_should_return_dashboards_from_all_users(
        self, account, other_account
    ):
        AccountDashboard.objects.create(account=account, name="Mine", num_columns=3)
        AccountDashboard.objects.create(
            account=other_account, name="Theirs", num_columns=3
        )
...
def other_account(db):
    other = Account(login="otherdashtest", name="Other Test User")
    other.save()
    yield other
    other.delete()
