...
class TestSetDefaultDashboardView:
    def test_given_dashboard_then_it_should_succeed(self, db, client, admin_account):
        ...
        assert f"Default dashboard set to «{dashboard.name}»" in smart_str(
            response.content
        )
        assert dashboard.is_default_for_account(admin_account)

    def test_given_dashboard_with_previous_default_dashboard_then_it_should_succeed(
...
class TestDeleteDashboardView:
    ...
    def test_given_last_dashboard_of_account_then_render_error_message(
        ...
        AccountDashboard.objects.filter(account=admin_account).delete()
        dashboard = create_dashboard(admin_account, name="last_dashboard")
    ...
    def test_given_default_dashboard_of_account_then_render_error_message(
        ...
        create_dashboard(admin_account, name="non_default_dashboard")
        default_dashboard = admin_account.default_dashboard
...
class TestRenameDashboardView:
    ...
    def test_given_dashboard_id_with_whitespace_new_name_then_show_error_message(
        self, db, client, admin_account
    ):
        dashboard = create_dashboard(account=admin_account)
        old_name = dashboard.name
...
class TestImportDashboardViews:
    ...
    def test_given_a_valid_dashboard_file_thencreate_dashboard(
