...
class TestListCommand:
    ...
    def test_when_listing_all_then_it_should_include_dashboards_from_all_users(
        ...
        assert "Other Dashboard" in output
    ...
    def test_when_listing_with_user_filter_then_it_should_show_only_user_dashboards(
        ...
        output = capsys.readouterr().out
        assert "CLI Dashboard" in output
        assert "Other Dashboard" not in output
    ...
    def test_when_listing_as_json_then_it_should_output_valid_json(
        ...
        other = next(d for d in data if d["name"] == "Other Dashboard")
        assert other["account"] == "otheruser"
...
@pytest.fixture
def other_user_dashboard(db):
    """A dashboard owned by a different user than the `account` fixture."""
    other = Account(login="otheruser", name="Other Test User")
    other.save()
    dashboard = AccountDashboard.objects.create(
        account=other, name="Other Dashboard", num_columns=2
    )
    yield dashboard
    other.delete()
