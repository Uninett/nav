...
def handler(req):
    ...
    database.execute("select priority, facility, mnemonic, type from message_type order by priority, facility, mnemonic")
    types = []
    typeid2type = {}
    type2typeid = {}
    types.append((0,"(All)","",""))
    ...
    if not error and not exception:
        ...
        if origin and type or origin and log or type and log:
            ...
            database.execute("select time, origin, newpriority, priority, facility, mnemonic, message from log_message inner join message_type USING (type) inner join origin USING (origin) where %s order by time desc" % where)
