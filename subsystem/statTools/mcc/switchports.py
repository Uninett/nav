...
logger = logging.getLogger("mcc.switch-ports")
...
def make_config(config):
    ...
    for (netboxid, sysname, ip, ro, snmp_version) in c.fetchall():
        ...
        interfacesq = """
        SELECT swportid, interface, ifindex, portname
        FROM netbox
        JOIN module USING (netboxid)
        JOIN swport USING (moduleid)
        WHERE netboxid = %s
        ORDER BY moduleid, port
        """
        ...
        # Fill in targets
        for (swportid, interface, ifindex, portname) in c.fetchall():
            if not interface:
            ...
            # key - swport
            # value - swportid
            container = cricket.RRDcontainer(targetname + '.rrd',
